...
//  construct probe positions 
    ...
    for (i=1;i<=25;i++) {
        printf("probe %d location r=%g, theta=%g \n",i,radii[i],angles[i]);
    }
...
//        fscanf (input, "%d \t %d \t %d \t %g", &id, &ma, &na, &tp); 
        fscanf (input, "%g", &tp); 
        printf("%d, %d, %g \n", id, ccidx[ma][na], tp);
//        if (tp!=tp) printf("%d, %d, %g \n", id, ccidx[ma][na], tp);
        if (id==1) {
//            printf("cc \n");
            cc[ccidx[ma][na]]=tp;
        }
...
//    datac=fopen("data40.txt", "r"); // open data for counting
    ...
    while(fscanf(datac, "%g", &datatmp)>0) {
    nphi++;
    }
    ...
    for (id=1;id<=nphi;id++) { // loop over azimuthal slices 
        fscanf (datafile, "%g", &dataread[0]); // read azimuthal angle in degrees 
        phi=dataread[0]*M_PI/180.;
        for (prb=1;prb<=25;prb++){ // loop over 25 probes 
            fscanf (datafile, "%g", &dataread[prb]); 
...
//            printf("%g \n",bsize);
        ...
        printf("Slice %d (phi=%g) of %d done (%g, %g, %g) \n",
    ...
    printf("RMS fluctuation = %g ppm \n", ffrms);
    printf("RMS difference (lin. approx.) = %g ppm \n", dzrms);
    printf("RMS difference (real) = %g ppm \n", difrms);
    /* calculate coeffs */
/*
    cterm=0.;
    printf ("\nComputing b-fields\n");
    for (id=1;id<=ndata;id++) { // loop over azimuthal slices 
        fscanf (input, "%g", &datatmp); // read azimuthal angle in degrees 
...
/*
    ...
    fprintf(out0, "NMax=%d;\n", nmax);
    fprintf(out0, "MMax=%d;\n", mmax);
    fprintf(out0, "csqc0=%.20g;\n", cterm);
    fprintf(out0, "csqc=(%.20g)+\n", cterm);

    for (i=1;i<=dim1;i++) {
        fprintf(out0, "(%.20g)*CC[%d,%d]+ \n", \
                cc[i],ccidx[i][1],ccidx[i][2]);
        if (ccidx[i][2]!=0) fprintf(out0, "(%.20g)*CS[%d,%d]+ \n", \
                cs[i],ccidx[i][1],ccidx[i][2]);
        if (ccidx[i][1]!=0) fprintf(out0, "(%.20g)*SC[%d,%d]+ \n", \
    ...
    for (i=1;i<=dim2;i++) {
        fprintf(out0, "(%.20g)*CC[%d,%d]*CC[%d,%d]+ \n",\
