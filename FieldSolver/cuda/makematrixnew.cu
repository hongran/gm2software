 ...
 */
__global__ void vectorProd(const double *d_tall, double *d_M, int dim_M)
{
  int i = blockDim.x * blockIdx.x + threadIdx.x;
  int j = blockDim.y * blockIdx.y + threadIdx.y;

  if (i < dim_M && j< dim_M){
    d_M[i*dim_M+j]+=d_tall[i]*d_tall[j];
  }
}
...
void cpx(double **datain, 
  ...
  tall=(double *)malloc(sizeof*tall*(dim1*4));
...
/*  err = cudaMalloc((void **)&d_tcc, d_size);
  ...
  err = cudaMalloc((void **)&d_tall, d_size);
...
/*	err = cudaMemcpy(d_tcc, tcc, d_size, cudaMemcpyHostToDevice);
	...
	err = cudaMemcpy(d_tall, tall, d_size, cudaMemcpyHostToDevice);

	dim3 DimBlock (16,16);
	dim3 DimGrid (4*dim1/16+1, 4*dim1/16+1);
//	printf("CUDA kernel launch with %d blocks of %d threads\n", DimGrid.x * DimGrid.y, 256);

	vectorProd<<<DimGrid, DimBlock>>>(d_tall, d_M, dimM);
